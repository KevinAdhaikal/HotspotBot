module.exports = {
    async execute(bot, msg, userData) {
        if (!/\d{1,}[gkm]$/i.test(msg.text)) {
            ...
            return await bot.sendMessage(msg.chat.id, `Mohon input Limit Bytes Total yang benar, Silahkan Input lagi\nContoh: 100M`, {
                reply_markup: {
                    inline_keyboard: [
        ...
        userData.editUser.limitBytesTotal = msg.text
        bot.sendMessage(msg.chat.id, `Limit Bytes Total telah di set ke ${msg.text}!\n\nKamu berada di Menu Tambah User Limits`, {
            reply_markup: {
                inline_keyboard: [
